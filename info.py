...
CHANNELS = [ch for ch in environ.get('CHANNELS', '-1002166283591').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002070438514') #Channel / Group Id for force sub ( make sure bot is admin )
...
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002130134772') # request channel id ( make sure bot is admin )
...
DUMP_CHNL = int(environ.get('DUMP_CHNL', '-1002353224172'))
