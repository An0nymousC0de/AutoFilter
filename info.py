...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002201654960').split()]
CHANNELS = [ch for ch in environ.get('CHANNELS', '-1002201654960').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002353224172') #Channel / Group Id for force sub ( make sure bot is admin )
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002186197570') # support group id ( make sure bot is admin )
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002201654960') # request channel id ( make sure bot is admin )
...
AI_LOGS = int(environ.get("AI_LOGS", "-1002332284740")) #GIVE YOUR NEW LOG CHANNEL ID TO STORE MESSAGES THAT THEY SEARCH IN BOT PM.... [ i have added this to keep an eye on the users message, to avoid misuse of Bot ]

#Auto approve 
CHAT_ID = [int(app_chat_id) if id_pattern.search(app_chat_id) else app_chat_id for app_chat_id in environ.get('CHAT_ID', '').split()]
TEXT = environ.get("APPROVED_WELCOME_TEXT", "<b>{mention},\n\nʏᴏᴜʀ ʀᴇǫᴜᴇsᴛ ᴛᴏ ᴊᴏɪɴ {title} ɪs ᴀᴘᴘʀᴏᴠᴇᴅ.\n\‣ ᴘᴏᴡᴇʀᴇᴅ ʙʏ @allmovieshub_official</b>")
APPROVED = environ.get("APPROVED_WELCOME", "on").lower()

# Referal Settings
REFERAL_COUNT = int(environ.get('REFERAL_COUNT', '20')) # number of referal count
REFERAL_PREMEIUM_TIME = environ.get('REFERAL_PREMEIUM_TIME', '1 week')
OWNER_USERNAME = environ.get('OWNER_USERNAME', 'imakashrabha') # owner username without @

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002125006518')) #Log channel id ( make sure bot is admin )
DUMP_CHNL = int(environ.get('DUMP_CHNL', '-1002459394931'))

# Verify
VERIFY = bool(environ.get('VERIFY', False)) # Verification On ( True ) / Off ( False )
HOWTOVERIFY = environ.get('HOWTOVERIFY', 'https://t.me/allmovieshub_official') # How to open tutorial link for verification
...
S_CHANNEL = environ.get('S_CHANNEL', "@allmovieshub_official")
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/Your_Movies_Group')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/allmovieshub_official')
...
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/Your_Movies_Group') #Support group link ( make sure bot is admin )
...
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002040878546'))
